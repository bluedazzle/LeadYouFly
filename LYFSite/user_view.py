...
def big_wheel(req):
    ...
    if not code:
        ...
        content = '''[{"id":0,"prize":"大奖降临：大奖降临:雷蛇 Taipan 太攀皇蛇游戏鼠标","v":0},{"id":1,"prize":"一等奖：LOL1000点券点卡","v":0.001},{"id":2,"prize":"二等奖：LOL500点券点卡","v":0.002},{"id":3,"prize":"三等奖：LOL双倍经验卡一日","v":1.0},{"id":4,"prize":"幸运奖：LOL100点券点卡","v":8.0}]'''
...
def get_reward_list(req):
    ...
    for itm in reward_list:
        ...
        data.append(body)
    return HttpResponse(json.dumps(data, ensure_ascii=False, indent=4), content_type='application/json')
